...
from .query_strategy import ExperimentQueryStrategy, DataSourceQueryStrategy, UTC
...
class ExperimentsearchTestCase(TestCase):
    ...
    def setUp(self):
        resource_path = urlparse.urljoin(
            'file:', urllib.pathname2url(os.getcwd()) + test_resources_path
        )
    ...
    def test_bad_url_2(self):
        querier = QueryMaker(ExperimentQueryStrategy())
        bad_url = urlparse.urljoin(
            'file:', urllib.pathname2url(os.getcwd()) + '/nonexistentdir/'
        )
        with self.assertRaises(QueryError):
    ...
    def test_index_response_1(self):
        ...
        for row in range(0, len(actual_table.rows)):
            ...
            for col in range(0, len(Experiment.field_names)):
                ...
                self.assertEqual(actual_row[field], expected_row[field])

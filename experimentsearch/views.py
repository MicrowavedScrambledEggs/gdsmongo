...
class IndexHelper:
    ...
    def search_by_date(self):
        ...
        if self.form.is_valid():
            ...
            # 'from' and 'to' dates
            dates = self.get_date_dict()  # Can't get dates from form for some reason
            self.search_list = Experiment.objects.filter(
    ...
    def get_date_dict(self):
        to_month = self.request.GET['to_date_month']
        to_day = self.request.GET['to_date_day']
        to_year = self.request.GET['to_date_year']
        from_month = self.request.GET['from_date_month']
        from_day = self.request.GET['from_date_day']
        from_year = self.request.GET['from_date_year']
        to_date_str = to_month + " " + to_day + " " + to_year
        from_date_str = from_month + " " + from_day + " " + from_year
...
def stream_experiment_csv(request, experi_name):
    ...
    """
    # Make query
    urllib.urlretrieve(genotype_url + experi_name, genotype_file_name)
